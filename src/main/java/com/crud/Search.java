...
public class Search extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement pd = conn.prepareStatement("SELECT *FROM winter_internship WHERE cust_number = ?");
			pd.setInt(1,Integer.parseInt(request.getParameter("cust_number")));
